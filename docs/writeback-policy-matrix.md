...
| `github` | `github_issue_create` | B for allowlisted internal repos; C otherwise | Preview-only proposal implemented; executor not implemented. | New issue creation from a Company Brain WorkItem or GuidanceItem. v0 generates an `ExternalActionProposal` with payload (repo, title, body with idempotency marker, labels, milestone), records `github_issue_create_previewed` in the audit trail and reports `executionBlocked=true`. Real executor (allowlist via `AIOS_GITHUB_ISSUE_CREATE_ALLOWLIST`, `GITHUB_TOKEN`, HITL approval, preview-after-approval, Retry Safety, marker-based idempotency dedup, audit) is left for a separate cut. Closing, reopening, editing or merging existing issues remain out of scope. |
