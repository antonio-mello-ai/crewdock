...
Status Production Operating Loop v0 em 2026-05-07: implementado runner recorrente interno no daemon, configuravel por `AIOS_COMPANY_BRAIN_OPERATING_LOOP_*`, sem cron/systemd timer novo e sem writeback externo. O loop checa watchers due/stale, roda somente `watcher-aios-briefing-v0` e `watcher-github-pr-ci-v0` com `actionPolicy=observe_only`, usa lock em memoria e grava provenance `schedule://...`. Exposto em `/api/company-brain/operating-loop`, `operatingCadence.operatingLoop`, `coreReadiness.operatingLoop`, Operating UI e MCP `get_company_brain_operating_loop`. Dogfood local validou run automatico com `watcherRunsCreated=2`, `artifactsCreated=2`, `activeScheduledWatcherCount=2`, `staleCadenceCount=0`, `dueCadenceCount=0` e `dailyUseBlockingGapCount=0`.

Proximo corte planejado: deployar Production Operating Loop v0 em CT165, habilitar `AIOS_COMPANY_BRAIN_OPERATING_LOOP_ENABLED=true` no `.env.prod`, validar que o loop recorrente zera `staleCadenceCount/dueCadenceCount` sem run manual e entao usar `https://ai.felhen.ai/company-brain/operating` como ciclo diario persistente. Continuar automaticamente apenas para deploy/validacao deste corte e depois read-only/audit/docs/UI/MCP/dogfood; parar antes de novo executor real, novo alvo externo, secrets/permissoes ou qualquer mutacao fora da policy aprovada.
