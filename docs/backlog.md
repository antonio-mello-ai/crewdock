...
Proximo corte planejado: consumir a proxima issue aberta da milestone `AIOS Execution Loop v0`, atualmente `#30 AIOS-EXEC-04: Session Result Intake`. Continuar automaticamente em cuts cobertos por policy/read-only/audit/docs/UI/MCP/dogfood; para mutacoes externas novas, usar executor allowlisted + HITL + preview-after-approval + Retry Safety, sem parar por default quando o alvo for repo interno privado e a policy ja estiver documentada. Para AIOS-EXEC-03 v1 implementar antes de retomar runner: schema + WorkflowLoader + workspace manager + WORKFLOW.md skeleton, sem rodar agentes.
...
- [ ] **AIOS-EXEC-03 v1 Agent Runner schema + WorkflowLoader + workspace manager** — adicionar `cbAgentRuns`, `cbAgentRunSteps`, `cbAgentRunRetries`, `cbAgentRunWorkspaces`, `WorkflowLoader` com Liquid renderer, workspace manager com git worktree e safety invariants, `WORKFLOW.md` skeleton no repo. Sem rodar agente.
- [ ] **AIOS-EXEC-03 v2 Orchestrator + AgentRunner subprocess + retry queue** — orchestrator acoplado ao Operating Loop, agent subprocess launcher (claude -p / codex app-server), retry/backoff Symphony, manual run via API/MCP. Off-by-default em producao via env flag.
- [ ] **AIOS-EXEC-03 v3 Auto-dispatch + reconciliation + UI** — habilitar dispatch automatico, stall detection, tracker refresh, workspace cleanup, UI Agent Runs, card no Operating Snapshot, integracao com Company Operating Map.
- [ ] **AIOS-EXEC-04 Session Result Intake** — transformar resultado de sessao/run em Artifact/Signal/Finding/Guidance
- [ ] **AIOS-EXEC-05 Company Operating Map v0** — mostrar mapa visual por area/projeto com WorkItems, agent runs, PR/CI, gates, blockers, source health, evidence e handoff
- [ ] **AIOS-EXEC-06 Company Command Router v0** (`#37`) — transformar solicitacao humana em area, blueprint, WorkItem, agent run ou ExternalActionProposal governada
- [ ] **AIOS-EXEC-07 Area Blueprint Registry v0** (`#38`) — registrar areas/departamentos, owners, agentes e blueprints iniciais para expandir alem de Development
- [ ] **AIOS-EXEC-08 Goal-to-Execution Superoptimizer v0** (`#39`) — decompor objetivos de negocio em areas, metricas, work items, watchers, agent runs, gates e cadence
- [ ] **AIOS-EXEC-09 Agent Run Evaluation Loop v0** (`#40`) — avaliar traces/falhas/resultados de runs agentic e gerar signals, guidance e improvement proposals
