...
- [x] **Auth em endpoints do daemon** — `api.crewdock.ai` atrás de CF Access self-hosted. Daemon valida JWT via `jose.createRemoteJWKSet` (RS256, iss, aud, clockTolerance 30s). Duas policies: email allow (users interativos) + service token allow (MCP server). `/api/health` público (bypass app separado). `/api/ping` serve iframe warmup para resolver cookie cross-origin. Loopback bypass para debug local no CT165. Ver `docs/auth-rollout.md`
