...
Status AIOS-EXEC-02 WorkItem to GitHub Issue Flow v0 em 2026-05-07: issue `#28` consumida em modo preview-only, seguindo o padrao `github_label_proposal_v0` (preview antes do executor real). Tipos novos no shared (`GitHubIssueCreateTarget`, `GitHubIssueCreateProposalPreviewResponse`, `GenerateGitHubIssueCreateProposalRequest`) e action kind `github_issue_create` em `ExternalActionKind`. Daemon ganhou helpers (`gitHubIssueCreateMarker`, `gitHubIssueCreateTarget`, `gitHubIssueCreatePayloadLabels`, `gitHubIssueCreateAllowlist`, `isGitHubIssueCreateRepoAllowlisted`), `buildGitHubIssueCreateProposalPreview`, `validateGitHubIssueCreateProposalPreview` e `buildGitHubIssueCreatePayloadFromWorkItem` que monta title/body/labels/milestone a partir do WorkItem com marker de idempotency. Endpoints novos: `POST /api/company-brain/external-action-proposals/from-work-item/github-issue-create` (gera ou reusa proposal por hash de `repo+workItemId+title`) e `POST /api/company-brain/external-action-proposals/:id/github-issue-create/preview` (dry-run). MCP tools `create_company_brain_github_issue_create_proposal` e `preview_company_brain_github_issue_create_proposal`. `previewEventForProposal` reconhece `github_issue_create_previewed`. `docs/writeback-policy-matrix.md` lista o novo action_type como preview-only com executor pendente. Smoke local com seed Felhen Demo v0.1 validou criacao de proposal, preview com `executionBlocked=true`, audit trail com `proposal_created` + `github_issue_create_previewed`, e idempotency (`reused=true`) ao recriar mesmo workItem+repo. Sem writeback externo, sem executor real, sem deploy.
