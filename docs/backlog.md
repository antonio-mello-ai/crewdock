...
Proximo corte planejado: usar `https://ai.felhen.ai/company-brain/operating` como ciclo diario persistente e coletar friccoes reais antes de criar features novas. Continuar automaticamente apenas para read-only/audit/docs/UI/MCP/dogfood; parar antes de novo executor real, novo alvo externo, secrets/permissoes ou qualquer mutacao fora da policy aprovada.
