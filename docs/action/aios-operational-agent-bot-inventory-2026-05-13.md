...
Remediacao/ativacao executada apos o inventario inicial: `TGM-00` do Telegram Command Layer foi seedado live no Company Brain em 2026-05-13. Isso adicionou a source `Telegram Bot Gateway CT165` (`S0m6x7yd29Kj`), o artifact `28qggBUysYk5` e a guidance `oJ_V8tOZzMqo` para implementar o primeiro caminho `/aios` com Command Router preview. `TGM-01` tambem foi implementado live no script compartilhado dos bots em CT165 no mesmo dia, registrado no artifact `zbQze3u9wb4P`; a guidance `oJ_V8tOZzMqo` foi concluida e a proxima guidance aberta e `cYHp2xMoDPNn` para definir `/aios commit` Risk A. A primeira fatia de `TGM-03` tambem foi implementada: pedidos de briefing de marketing retornam um briefing operacional baseado na guidance aberta de marketing, registrado no artifact `TG26isZIoJnq`. A evolucao MKT-01 transformou esse briefing em output concreto com 3 prioridades, 12 acoes, abordagem para contabilidade e draft para Thais, registrada no artifact `yUH1XkSwX39y`. A persistencia automatica por briefing foi testada, mas a politica final foi corrigida: briefings sob demanda sao efemeros por padrao e so viram `marketing_briefing` via `/aios promover briefing [motivo]`. A primeira fatia de MKT-02 tambem esta ativa: `/aios feedback ...` cria artifact `marketing_feedback` e atualiza a guidance `R6adR2HkGBq0` apenas quando ha artifact promovido ou `artifactId` explicito. A evolucao atual moveu essa inteligencia para o daemon em `POST /api/company-brain/operating-packs/run`; o bot Telegram deve ficar como gateway e cache de contexto do chat, nao como dono do pack.

Naming operacional corrigido apos a revisao: o ID curto do Company Brain e tratado como chave tecnica; artifacts promovidos passam a expor nome canonico (`metadata.artifactName`), chave de agrupamento (`metadata.semanticKey`) e URI semantica (`rawRef`) para busca/QMD.
